"""Generate the v0.1 pen.dev design board from the documented UI contract.
...
C = {
    "paper": "#F7F8F6", "white": "#FFFFFF", "ink": "#142522",
    "muted": "#5E6C68", "faint": "#84918C", "line": "#DDE4DF",
    "accent": "#176B57", "accent_soft": "#E6F1EC", "nav": "#132825",
    "nav_muted": "#A8BBB2", "red": "#A8453B", "red_soft": "#F9ECE9",
    "amber": "#8D642B", "amber_soft": "#F7F0E4", "gray_soft": "#EEF1EF",
}
...
def ident(name):
    global count
    count += 1
    return f"n{count:04d}_{name.replace(' ', '_')}"
...
def shell(parent, title, role, active="任务大盘", width=1440):
    ...
    for i, label in enumerate(["任务大盘", "审查工作台", "交付与回写", "故障与恢复"]):
        yy = 142 + i * 53
        if label == active:
    ...
    text(parent, "nav demo notice", 28, 819, 178, "合成数据演示版 · v0.1 设计", 11, C["nav_muted"])
...
def status_trio(parent, x, y, machine, legal, writeback, w=1088):
    ...
    for i, (label, value) in enumerate([("机器审查", machine), ("法务复核", legal), ("模拟回写", writeback)]):
        ...
        text(parent, label + " title", xx, y + 12, col - 44, label, 11, C["muted"], "600")
        text(parent, label + " value", xx, y + 33, col - 44, value, 15, C["ink"], "700")
...
rect(p, "overview", 266, 169, 1128, 139, C["nav"], 14)
text(p, "overview heading", 294, 190, 600, "今天需要关注", 14, C["nav_muted"], "600")
text(p, "overview number", 294, 217, 270, "03", 42, C["white"], "700")
text(p, "overview caption", 366, 238, 280, "项任务有新进展", 15, C["white"])
rect(p, "overview division", 733, 192, 1, 92, "#45625A")
text(p, "overview note", 770, 200, 565, "当前版本与历史确认结果分开显示。\n处理中的机器结果不会提前当作正式结论。", 16, C["white"])
text(p, "task section", 266, 343, 480, "我的合同", 20, C["ink"], "700")
text(p, "table count", 1296, 349, 98, "共 4 项", 12, C["muted"])
rect(p, "filter status", 266, 392, 184, 38, C["white"], 9, C["line"])
text(p, "filter status text", 281, 402, 150, "状态：全部  ▾", 13, C["ink"])
rect(p, "filter risk", 462, 392, 184, 38, C["white"], 9, C["line"])
text(p, "filter risk text", 477, 402, 150, "风险：全部  ▾", 13, C["ink"])
rect(p, "table", 266, 449, 1128, 373, C["white"], 12, C["line"])
rect(p, "table header", 267, 450, 1126, 48, C["gray_soft"], 11)
for xx, label in [(288, "合同 / 申请人"), (664, "金额"), (782, "机器审查"), (916, "法务复核"), (1044, "模拟回写"), (1192, "当前正式等级")]:
    text(p, "column " + label, xx, 464, 184, label, 12, C["muted"], "600")
rows = [
