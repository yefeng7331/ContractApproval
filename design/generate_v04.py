"""Generate the v0.4 pen.dev design board from the documented UI contract.
...
def shell(parent, title, role, active="任务大盘", width=1440):
    ...
    text(parent, "nav demo notice", 28, 819, 178, "合成数据演示版 · v0.4 设计", 11, C["nav_muted"])
...
text(p, "dashboard hint", 267, 105, 735, "本人任务的当前进度、下一步与已确认结果", 14, C["muted"])
button(p, "import mock", 1162, 103, 147, "导入模拟待办")
button(p, "upload", 1321, 103, 227, "上传合成合同", True)
text(p, "overview heading", 266, 166, 480, "当前处理分布", 22, C["ink"], "700")
text(p, "overview scope", 844, 174, 244, "本人任务 · 共 4 份", 12, C["muted"])
rule(p, "overview top rule", 266, 208, 822, C["line_strong"])
text(p, "overview note", 267, 223, 790,
     "按当前文档的主阶段归类；旧版已确认结果不计入当前分布。", 12, C["muted"])
stages = [(292, "机器处理中", "1 份 · 25%", C["line_strong"]),
          (485, "待法务复核", "1 份 · 25%", C["amber"]),
          (678, "法务已确认", "1 份 · 25%", C["accent"]),
          (871, "处理受阻", "1 份 · 25%", C["red"])]
for xx, label, detail, color in stages:
    rect(p, "stage segment", xx, 258, 190, 23, color, 4)
    text(p, "stage name", xx, 299, 182, label, 14, C["ink"], "600")
    text(p, "stage count", xx, 325, 182, detail, 12, C["muted"])
rule(p, "overview bottom rule", 266, 375, 822, C["line_strong"])

rect(p, "next action surface", 1112, 166, 436, 209, C["white"], 12, C["line"])
text(p, "next action heading", 1137, 185, 208, "下一步", 19, C["ink"], "700")
pill(p, "next action count", 1382, 183, 141, "1 项需处理", C["red_soft"], C["red"])
rule(p, "next action divider", 1137, 229, 386)
text(p, "next action task", 1137, 246, 382, "软件采购合同 D", 17, C["ink"], "700")
text(p, "next action reason", 1137, 277, 382, "文档 V1 · 附件加密，解析受阻", 12, C["red"], "600")
text(p, "next action recovery", 1137, 303, 382, "本人可上传修订附件，旧版仍保留。", 12, C["muted"])
button(p, "next action open", 1352, 330, 171, "查看恢复路径", True)

text(p, "task section", 266, 405, 480, "合同明细", 20, C["ink"], "700")
text(p, "table count", 1450, 411, 98, "共 4 项", 12, C["muted"])
rect(p, "filter status", 266, 448, 184, 38, C["white"], 9, C["line"])
text(p, "filter status text", 281, 458, 112, "状态：全部", 13, C["ink"])
text(p, "filter status action", 405, 458, 34, "更改", 12, C["accent"], "600")
rect(p, "filter risk", 462, 448, 184, 38, C["white"], 9, C["line"])
text(p, "filter risk text", 477, 458, 112, "风险：全部", 13, C["ink"])
text(p, "filter risk action", 601, 458, 34, "更改", 12, C["accent"], "600")
text(p, "filter description", 1011, 458, 537,
     "列表筛选不改变上方概览口径", 12, C["muted"])
rect(p, "table", 266, 502, 1282, 320, C["white"], 12, C["line"])
rect(p, "table header", 267, 503, 1280, 43, C["gray_soft"], 11)
...
for xx, ww, label in columns:
    text(p, "column " + label, xx, 516, ww, label, 12, C["muted"], "600")
rows = [
    ("软件采购合同 A", "张明", "软件采购", "¥ 480,000", "09-23 10:32", "草稿完成", "待复核", "未回写", "—", "文档 V2 · 旧版已确认 V1"),
    ("软件采购合同 B", "张明", "软件采购", "¥ 260,000", "09-22 15:10", "已完成", "已确认", "模拟成功", "高", "文档 V1 · 审查 V1"),
    ("软件采购合同 C", "张明", "软件采购", "¥ 320,000", "09-23 09:18", "解析中", "待复核", "未回写", "—", "文档 V1 · 当前无结论"),
    ("软件采购合同 D", "张明", "软件采购", "¥ 175,000", "09-22 11:45", "受阻", "待复核", "未回写", "—", "文档 V1 · 附件加密"),
]
for i, row in enumerate(rows):
    yy = 546 + i * 69
    if i:
        rule(p, "row divider", 283, yy, 1248)
    text(p, "contract", 288, yy + 10, 239, row[0], 13, C["ink"], "600")
    text(p, "version note", 288, yy + 34, 239, row[9], 11, C["muted"])
    for (xx, ww, _), val in zip(columns[1:], row[1:9]):
        color = C["red"] if val in ("受阻", "高") else C["accent"] if val in ("已确认", "模拟成功") else C["ink"]
        text(p, "row state", xx, yy + 22, ww, val, 12, color, "600")
...
# 07 — The visual system makes v0.4's hierarchy and states explicit
p = frame(children, "07 视觉规范 · v0.4", 0, 2080, 1440, 900, C["white"])
text(p, "system heading", 64, 50, 980, "合同审查 · 界面规范", 31, C["ink"], "700")
pill(p, "system version", 1234, 54, 140, "DESIGN  v0.4", C["accent_soft"], C["accent"])
...
p = frame(children, "09 关键状态与恢复 · v0.4", 3360, 2080, 1440, 900, C["white"])
...
p = frame(children, "10 操作反馈与编辑保护 · v0.4", 0, 3120, 1440, 900, C["paper"])
text(p, "feedback heading", 64, 47, 1040, "每一步，都有明确反馈。", 32, C["ink"], "700")
pill(p, "feedback version", 1246, 55, 130, "DESIGN v0.4", C["accent_soft"], C["accent"])
