...
| `POST /tasks` | `multipart/form-data`：`file`、`department`、`applicant` → `task_id`、文档版本、`pending`；仅合成 DOCX/PDF/PNG/JPEG/TIFF，每份最多 25 MiB；接入时检查文件格式特征和摘要，深度解析留待处理器 | 业务经办人；接入和落库已实现，后台异步解析待实施，当前任务保持 `pending` |
